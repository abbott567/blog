...
export default {
  is: {
    ...
    undefined: () => {
      ...
      expect(() => new Post(t1)).to.throw('meta.description undefined when constructing Post')
    },
    blank: () => {
      Post.clean()
      const t1 = deepClone(params)
      t1.title = 'Meta description is blank'
      expect(() => new Post(t1)).to.throw('meta.description blank when constructing Post')
    },
    not: {
      a: {
        string: () => {
          ...
          expect(() => new Post(t1)).to.throw('meta.description not a string when constructing Post')
        }
      }
    }
  }
